CREATE OR REPLACE FUNCTION "public"."cms_page_tree_published"("page_id" INTEGER, "lang" TEXT) RETURNS BOOLEAN LANGUAGE sql stable AS $$
select
...
alter table public.cms_page_texts add column parents_published boolean not null default true;
alter table public.cms_page_texts alter column parents_published drop default;

update cms_page_texts t set parents_published = x.parents_published
from (select page_id, cms_page_tree_published(parent, lang) as parents_published
      from cms_pages join cms_page_texts using (page_id)) as x
where t.page_id=x.page_id and t.parents_published != x.parents_published;

drop view cms_v_pages cascade;
