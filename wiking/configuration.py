...
# Components (Python packages) providing runtime data (resources, translations,
# documentation) are the framework baseline plus the packages containing the
# application modules configured in the 'modules' option (including their
# extension subpackages).
_FRAMEWORK_COMPONENTS = ('wiking', 'lcg', 'pytis')


def _is_package(name):
    """Return true if 'name' is an importable Python package (not a plain module)."""
    try:
        module = importlib.import_module(name)
    except ImportError:
        return False
    return bool(module.__file__) and os.path.basename(module.__file__).startswith('__init__.')


def _components(modules):
    """Return component package names contributing runtime data, most specific first.

    For each configured module, its containing packages are collected (e.g.
    'biblio.catalog.catalog' contributes 'biblio.catalog' and 'biblio'), together
    with the framework baseline.  Ordered most-specific first, so that an extension
    overrides its package and applications override the framework.

    """
    result = []
    for module_name in tuple(modules) + _FRAMEWORK_COMPONENTS:
        parts = module_name.split('.')
        for i in range(len(parts), 0, -1):
            name = '.'.join(parts[:i])
            if name not in result and _is_package(name):
                result.append(name)
    result.sort(key=lambda name: name.count('.'), reverse=True)
    return result


def _component_dirs(modules, subdir):
    """Return (component, directory) pairs of existing '<package>/<subdir>'
    directories across all components (see '_components'), most specific first."""
    result = []
    for component in _components(modules):
        directory = os.path.join(os.path.dirname(importlib.import_module(component).__file__),
                                 *subdir.split('/'))
        if os.path.isdir(directory):
            result.append((component, directory))
    return result
